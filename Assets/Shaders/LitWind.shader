...
Shader "Custom/Lit Wind"
{
	Properties
    {
        ...
        _OriginY("Origin Y", Float) = 0.2
        _DistanceMultiplier("Distance Multiplier", Float) = 2.0
        _Direction("Direction (Radians)", Float) = 0.0
        _OriginDirection("OriginDirection", Float) = 1.0 // 1 = up, 0 = down
		...
		Pass
        {
			...
			Varyings LitPassVertex(Attributes IN)
            {
                ...
                if (condition)
                {
                    ...
                    float cosDir = _CosDirection;
                    float sinDir = _SinDirection;

                    float newFreq = _Frequency / 2 + SimpleNoise(IN.uv, 1) / 2;
                    float randomValue = Rand(IN.positionOS.xy) * 0.01;

                    float t = _TimeScale * _Time.y;
                    float angle = t * newFreq + IN.positionOS.y * PI;
                    ...
                    float displacementX = _DistanceMultiplier * (distanceFromOrigin * distanceFromOrigin) * (directionalStaticX + intensityX);
                    float displacementZ = _DistanceMultiplier * (distanceFromOrigin * distanceFromOrigin) * (directionalStaticZ + intensityZ);
                ...
                half3 vertexLight = VertexLighting(worldPos, normalInputs.normalWS);
